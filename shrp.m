function [f0_time,f0_value,SHR,f0_candidates]=shrp(Y,Fs,F0MinMax,...
    frame_length,timestep,SHR_Threshold,ceiling,med_smooth,CHECK_VOICING)
...
N=N-m;
N=N*4;
%In fact, in most cases we don't need to multiply N by 4 and get
%equally good results yet much faster.

% derive how many frames we have based on segment length and timestep.
segmentlen=round(segmentduration*(Fs/1000));%get the length of every frames
inc=round(timestep*(Fs/1000));%noverlap length
nf = fix((total_len-segmentlen+inc)/inc); %the number of frames
n=(1:nf);
f0_time=((n-1)*timestep+segmentduration/2)'; % anchor time for each frame, the middle point
%f0_time equals the start time of every frames
...
frequency = Fs*(1:fftlen/2)/fftlen;
% we ignore frequency 0 here since we need to do log transformation 
%later and won't use it anyway.
...
startpos=shift_units+1-round(log2(i)/min_bin);
% find out all the start position of each shift
index=find(startpos<1); 
...
interp_len=length(interp_logf);
% new length of the amplitude spectrum after interpolation
totallen=shift_units+interp_len;
endpos=startpos+interp_len-1;
% note that:totallen=shift_units+interp_len;
index=find(endpos>totallen);
endpos(index)=totallen; 
% make sure all the end positions not greater than the totoal length of the shift spectrum

newfre=2.^(interp_logf); 
% the linear Hz scale derived from the interpolated log scale
upperbound=find(interp_logf>=maxlogf); 
% find out the index of upper bound of search region on the log frequency scale.
upperbound=upperbound(1);
% only the first element is useful
lowerbound=find(interp_logf>=minlogf); 
% find out the index of lower bound of search region on the log frequency scale.
lowerbound=lowerbound(1);

%----------------- segmentation of speech ------------------------------
curpos=round(f0_time/1000*Fs);
...
if (CHECK_VOICING)
    NoiseFloor=ethreshold(frames);
    NoiseFloor = 0.1;
    voicing=vad(frames,segmentduration/1000,NoiseFloor,minzcr);
else
    voicing=ones(nf,1);
end
%------------------- the main loop -----------------------
...
cur_cand2=0;
for n=1:nf
    segment=frames(n,:);%one frames of speech singal
    curtime=f0_time(n);
    if voicing(n)==0 %
        curf0=0;
        cur_SHR=0;
        ...
        [peak_index,cur_SHR,shshift,all_peak_indices]=ComputeSHR(log_spectrum,min_bin,startpos,endpos,...
            lowerbound,upperbound,N,shift_units,SHR_Threshold);
        if (peak_index==-1)
            % -1 indicates a possibly unvoiced frame, if CHECK_VOICING, set f0 to 0, otherwise uses previous value
            if (CHECK_VOICING)
                curf0=0;
                cur_cand1=0;
                cur_cand2=0;
...
%             end
            ...
            if (CHECK_VOICING)
                voicing(n)=postvda(segment,curf0,Fs,0.2,minzcr);
                if (voicing(n)==0)
                    curf0=0;
                end
            end
...
amplitude = abs(Spectra(1:fftlen/2+1)); 
% fftlen is always even here. Note: change fftlen/2 to fftlen/2+1. bug fixed due to Herbert Griebel
amplitude=amplitude(2:limit+1);
...
shshift=zeros(N,totallen); 
%initialize the subharmonic shift matrix; each row corresponds to a shift version
shshift(1,(totallen-len_spectrum+1):totallen)=log_spectrum; 
...
shshift=shshift(:,shift_units+1:totallen);
...
if (NumPitchCandidates == 1)
    % this is possible, mainly due to we put a constraint on search region, i.e., f0 range
...
%----------------------------------------------------------------------------------------
%-----------------------------------voicing determination -------------------------------
function voice=vad(x,segmentdur,noisefloor,minzcr)
...
voice=ones(nf,1);
energy=sum(x.^2,2);
index=find(energy<=noisefloor*3);
voice(index)=0;
zcr1 = zcr(x,segmentdur); 
index = find(zcr1>minzcr);
voice(index)=0;
...
function voicing=postvda(segment, curf0,Fs,r_threshold, minzcr)
% check voicing again using estimated F0,
%which follows Hermes, SHS algorithm, JASA, 1988
if nargin<4
    r_threshold=0.2;
end
estimated_period=1/curf0;
mid_point=round(length(segment)/2);
num_points=round(estimated_period*Fs);
...
if (start_point <1)
    ...
    if (mid_point>length(segment)) 
    % this is unreasonable, set f0 to zero
        voicing=0;
        return;
    end
end
...
[~,len]=size(x);
zcr=sum(0.5*abs(sign(x(:,2:len))-sign(x(:,1:len-1))),2)/dur;
%*************************************************************************************
%--------------------- Window function -------------------------------------------  
